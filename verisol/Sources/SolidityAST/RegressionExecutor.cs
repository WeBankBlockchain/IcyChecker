...
namespace SolidityAST
{
    ...
    class RegressionExecutor
    {
        ...
        public bool Execute(string filename)
        {
            ...
            // Console.WriteLine(sourceUnits);
            //TODO 
            Console.Write("end executor \n");
        ...
        private bool TextCompare(string originalText, string astText)
        {
            ...
            // Console.WriteLine(textWithoutComments);
            string[] separator = { "{", "}", ",", ".", ";", "(", ")", "^", "[", "]", " ", "\n", "\r", "\t" };
            ...
            List<string> enumKeywords = new List<string>() {
                "CONTRACT", "LIBRARY", "INTERFACE", "ENUM", "STRUCT",
            ...
            {
                Console.WriteLine(normalizedWords.Length + "," + astWords.Length);
                Console.WriteLine("normalizedWords != astWords length" );
                return false;
            }
            for (var i = 0; i < normalizedWords.Length; ++i)
            {
                if (enumKeywords.Contains(astWords[i]))
                {
                    ...
                    {
                        Console.WriteLine("!astWords[i].ToLower().Equals(normalizedWords[i])" );
                        return false;
                    }
                }
                else if (!normalizedWords[i].Equals(astWords[i]))
                {
                    Console.Write(normalizedWords[i] + astWords[i] );
                    Console.WriteLine("!normalizedWords[i].Equals(astWords[i]" );
                    return false;
                }
            }
            return true;
