...
namespace SolToBoogie
{
    ...
    public class ProcedureTranslator : BasicASTVisitor
    {
        ...
        // private MemberAccess nestedMember;
        // private Boolean IsNested;

        public List<string> translateMapping = new List<string>();
        ...
        private void TranslateStructDefinition(StructDefinition structDefn)
        {
            ...
            //big change、 regards udt as ref
            foreach(var member in structDefn.Members)
            {
                if(member.TypeDescriptions.IsStruct()){
                    var mapType = new BoogieMapType(BoogieType.Ref, BoogieType.Ref);
                    var mapName = member.Name + "_" + structDefn.CanonicalName ;
                    Console.WriteLine("TranslateStructDefinition:" + mapName); 
            ...
            //                     var mapType = new BoogieMapType(BoogieType.Ref, type);
            //                     var mapName = submember.Name + "_" + member.Name + "_" + structDefn.CanonicalName;
            //                     Console.WriteLine($"PRO1: type:{type}, \nmapName:{mapName}");
            //                     context.Program.AddDeclaration(new BoogieGlobalVariable(new BoogieTypedIdent(mapName, mapType)));
            //                 }
            //             }
            //         }
        ...
        private void GenerateInitializationForElementaryTypes(VariableDeclaration varDecl, ElementaryTypeName elementaryType)
        {
            ...
            else if (elementaryType.TypeDescriptions.TypeString.Equals("bool"))
            {
                string varName = TransUtils.GetCanonicalStateVariableName(varDecl, context);
                BoogieExpr lhs = new BoogieMapSelect(new BoogieIdentifierExpr(varName), new BoogieIdentifierExpr("this"));
                bool value = varDecl.Value != null && varDecl.Value.ToString() == "true";
        ...
        {
            // Console.WriteLine("GOTO TranslateExpr");
            currentExpr = null;
            ...
            if(expr.TypeDescriptions!=null) //  && currentExpr is BoogieIdentifierExpr)
            {
                ...
                try{
                    if(expr != null && expr.TypeDescriptions != null){
                        var translateItem = $"{currentContract.Name},{currentFunction.Name},{expr},{currentExpr}";
                        if( !translateMapping.Contains(translateItem)){
                            translateMapping.Add(translateItem);
                        }
                    }
                }
                catch (Exception ex){
        ...
        public override bool Visit(MemberAccess node)
        {
            ...
            // Console.WriteLine($"MeM1:Current_Node_Type: {node.Expression.GetType()}, Current_Node_Name:{node.MemberName}, {node.Expression.ToString()}");
            ...
            // Console.WriteLine($"MeM1:node.Expression.TypeDescriptions: {node.Expression.TypeDescriptions}");
            
            //use ref way
            if (node.Expression.TypeDescriptions.IsStruct())
            {
                var baseExpr = TranslateExpr(node.Expression);
                // #MemberAccess0: Expression: counter, struct Counters.NextCounter storage pointer
                // Console.WriteLine($"MemberAccess0: Expression: {node.Expression}, {node.Expression.TypeDescriptions}, type:{node.Expression.GetType()}");
                var memberMap = node.MemberName + "_" + node.Expression.TypeDescriptions.TypeString.Split(" ")[1];
                // Console.WriteLine(node.Expression.TypeDescriptions.TypeString);
                // MemberAccess1: baseExpr: counter_s21, memberMap: count_Counters.NextCounter
                // Console.WriteLine($"MemberAccess1: baseExpr: {baseExpr}, memberMap: {memberMap}");
                currentExpr = new BoogieMapSelect(
                    new BoogieIdentifierExpr(memberMap),
                    baseExpr);
                return false;
                /*
            // // split into simple types  
            // if (node.Expression.TypeDescriptions.IsStruct())
            // {
            //     if (node.Expression is MemberAccess)
            //     {
            //         nestedMember = node;
            //         IsNested = true;
            //     }
            ...
            //     {
            //         var baseExpr = TranslateExpr(node.Expression);
            //         Console.WriteLine($"MeM2:baseExpr: {baseExpr}");
            //         String memberMap;
            //         if (nestedMember != null)
            //         {
            //             memberMap = nestedMember.MemberName + "_" + node.MemberName + "_" +
            //                             node.Expression.TypeDescriptions.TypeString.Split(" ")[1];
            //         }
            //         else
            //         {
            //             memberMap = node.MemberName + "_" +
            //                             node.Expression.TypeDescriptions.TypeString.Split(" ")[1];
            //         }
            //         nestedMember = null;
            //         Console.WriteLine($"MeM2:memberMap: {memberMap}");
            //         currentExpr = new BoogieMapSelect(
            //             new BoogieIdentifierExpr(memberMap),
            //             baseExpr);
            //         Console.WriteLine($"MeM3:currentExpr: {currentExpr}");
            //     }
            //     
                return false;
                */
                /* process of procedure
                *   this.BaseExpr = baseExpr;; --memberMap
                    this.Arguments = new List<BoogieExpr>();
                    this.Arguments.Add(indexExpr); --baseExpr
                 */
            } 
