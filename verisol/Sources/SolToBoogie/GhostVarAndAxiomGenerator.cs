...
namespace SolToBoogie
{
    ...
    public class GhostVarAndAxiomGenerator
    {
        ...
        private void GenerateStructConstructors(ContractDefinition contract, StructDefinition structDefn)
        {
            ...
            foreach(var member in structDefn.Members)
            {
                if(member.TypeDescriptions.IsStruct() 
                   && member.TypeName is UserDefinedTypeName udt){
                    var formalType = BoogieType.Ref;
                    var formalName = member.Name;
                    Console.WriteLine("GenerateStructConstructors0:" + formalType + formalName);
            ...
            //TODO 
            //should we set for every var in struct or just ref 
            ...
            foreach (var member in structDefn.Members)
            {
                ...
                var formalName = member.Name;
                var mapSelectExpr =
                    new BoogieMapSelect(new BoogieIdentifierExpr(mapName), new BoogieIdentifierExpr("this"));
            ...
            //                     var formalName = submember.Name;
            //                     inParams.Add(new BoogieFormalParam(new BoogieTypedIdent(formalName, formalType)));
            //                     Console.WriteLine($"GHO1:formalName: {formalName}");
            //                 }
            //             }
            //         }
            //     }else{
            ...
            //                     Console.WriteLine($"GHO2:mapName: {mapName}\nformalName: {formalName}");
            //                 }
            //             }
            //         }
            //     }else{
