 ...
 * convert a Phase Event automaton object into a Dot-File. The dot-file can then be visualized in Graphviz;
 * Note that Graphviz cannot depict the math formulas(at least it cannot do so under windows); 
 * However you can convert via dot2tex the dotfiles to texfiles (using the pgf-library) and 
 * get a visualization via tex (including the math-formulas)
 * 
 * For GraphViz see: http://www.graphviz.org/
 * For Dot2Tex see: http://www.fauskes.net/code/dot2tex/documentation/
 * For PGF/TIK see: http://www.ctan.org/tex-archive/help/Catalogue/entries/pgf.html
 * 
 * The Converter is called, e.g. with:
 *  // DOTWriter dotwriter = new DOTWriter("C:/Test.dot", ctParallel);
    // dotwriter.write();
 * 
...
public class DOTWriter extends TCSWriter{
	...
	public DOTWriter(String destfile, boolean rename, PhaseEventAutomata pea) {
		this(destfile, pea);
        this.rename = rename;
	}
	...
	protected void writeDecision(Decision decision, int child, Writer writer)
	 ...
	 /**
	  *  instead of initial edges we construct a new initial node "init" and write edges to the 
	  *  initial nodes
	  *  
	  *  As Tex interprets "_" as command, we need to delete(or change) it from the names of the states.
	    ...
	    /**
		  *  To get the edge labels more readable I'm using \substack and \\\\ for new lines
		  *  As Tex interprets "_" as command, we need to delete(or change) it from the names of the states.
		     */
