...
public class NWAContainer extends ObjectContainer<INestedWordAutomaton<CodeBlock, String>>
		implements IVisualizable<VisualizationNode> {
	...
	public VisualizationNode getVisualizationGraph() {
		final Collection<String> initials = getValue().getInitialStates();

		final ArrayList<NWAVisualizationNode<String, CodeBlock>> visInitials = new ArrayList<>();
		for (final String initial : initials) {
			visInitials.add(new NWAVisualizationNode<>(getValue(), initial));
		}
