 ...
 * <br>
 *
 * Overview about the treatment of string literals in C according to the C11 standard: Summary of 6.4.5 for our
 * purposes:
 * <li>String literals have static storage duration. They are stored in their own memory area, writing to that area is
 * undefined behavior.
 * <li>When a string literal is stored, it is automatically terminated by a '\0'-character (in case it is stored into a
 * char[], the \0 may be dropped if the char[] is not large enough).
 * <li>The type of a string literal is char[].
 * <li>If a string literal is used for initializing a char[], its contents are copied into the char[]'s memory location.
 * (An exception to the 'no array assignments'-rule in C.) Assigning a string literal to a char[] outside of
...
public class StringLiteralResult extends ExpressionResult {
	 ...
	 * storage duration and thus are handled by StaticObjectsHandler. (whenever a StringLiteralResult is created, the
