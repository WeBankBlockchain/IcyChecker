...
/**
 * The {@link DebugObserver} allows you to inspect all the nodes of an AST.
 *
...
public class DebugObserver extends BaseObserver {
	...
	private void walkTree(final IWalkable root) {
		final Queue<IWalkable> queue = new ArrayDeque<>();
		final Set<IWalkable> processed = new HashSet<>();
		queue.add(root);
		while (!queue.isEmpty()) {
