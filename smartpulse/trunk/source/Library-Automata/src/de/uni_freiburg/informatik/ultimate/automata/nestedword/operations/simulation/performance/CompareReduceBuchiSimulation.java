...
//						ESimulationType.DELAYED, true, true, true)));
	 ...
	 *            Table to convert
	 */
	public static void tableToHtmlFile(final String name, final List<String> table) {
		...
		// Write html to file
		final Path resultDir = Paths.get(LOG_PATH.getAbsolutePath());
		try {
			Files.createDirectories(resultDir);
			final Path resultFile = resultDir.resolve(LOG_PATH_HTML_PRE + name + LOG_PATH_HTML_SUFF);
			Files.write(resultFile, Collections.singletonList(htmlText.toString()), StandardOpenOption.CREATE,
