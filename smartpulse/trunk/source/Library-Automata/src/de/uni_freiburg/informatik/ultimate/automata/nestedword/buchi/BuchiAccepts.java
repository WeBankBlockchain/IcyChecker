...
public final class BuchiAccepts<LETTER, STATE> extends AbstractAcceptance<LETTER, STATE> {
	...
	private boolean buchiAccepts() throws AutomataLibraryException {
		...
		do {
			...
			for (int i = 0; i < mLoop.length(); i++) {
				currentConfigs = successorConfigurations(currentConfigs, mLoop, i, mOperand, false);
	...
	private Set<STATE> computeHondaStates() throws AutomataLibraryException {
		...
		for (int i = 0; i < mStem.length(); i++) {
			currentConfigs = successorConfigurations(currentConfigs, mStem, i, mOperand, false);
			if (isCancellationRequested()) {
				throw new AutomataOperationCanceledException(this.getClass());
			}
		}
	...
	private boolean repeatedLoopLeadsAgainToHondaState(final STATE hondaState) throws AutomataLibraryException {
		...
		final Set<STATE> visitedatHondaAccepting = new HashSet<>();
		final Set<STATE> visitedatHondaNonAccepting = new HashSet<>();
		final Set<STATE> singletonStateSet = new HashSet<>();
		singletonStateSet.add(hondaState);
		final Set<ArrayDeque<STATE>> singletonConfigSet = emptyStackConfiguration(singletonStateSet);
		currentConfigsVisitedAccepting = removeAcceptingConfigurations(singletonConfigSet, mOperand);
		...
		while (!currentConfigsNotVisitedAccepting.isEmpty() || !currentConfigsVisitedAccepting.isEmpty()) {
			for (int i = 0; i < mLoop.length(); i++) {
				currentConfigsVisitedAccepting =
						successorConfigurations(currentConfigsVisitedAccepting, mLoop, i, mOperand, false);
				currentConfigsNotVisitedAccepting =
						successorConfigurations(currentConfigsNotVisitedAccepting, mLoop, i, mOperand, false);
	 ...
	 */
	private void removeAllWhoseTopmostElementIsOneOf(final Set<ArrayDeque<STATE>> configurations,
			final Set<STATE> states) {
		final List<ArrayDeque<STATE>> removalCandidate = new ArrayList<>();
